 ...
 *
 * @covers \tool_mucertify\external\form_source_cohort_edit_cohortids
 */
final class form_source_cohort_edit_cohortids_test extends \advanced_testcase {
    ...
    public function test_execution(): void {
        ...
        $result = form_source_cohort_edit_cohortids::execute('', $certification1->id);
        ...
        $result = form_source_cohort_edit_cohortids::execute('ta 1', $certification1->id);
        $this->assertNull($result['notice']);
        $expected = [
            ['value' => $cohort1->id, 'label' => $cohort1->name],
        ];
        ...
        $result = form_source_cohort_edit_cohortids::execute('', $certification2->id);
        $this->assertNull($result['notice']);
        $expected = [
            ['value' => $cohort2->id, 'label' => $cohort2->name],
            ['value' => $cohort3->id, 'label' => $cohort3->name],
    ...
    public function test_execution_tenant(): void {
        ...
        $result = form_source_cohort_edit_cohortids::execute('', $certification0->id);
        $this->assertNull($result['notice']);
        $expected = [
    ...
    public function test_validate_cohortid(): void {
        ...
        $this->assertNull(form_source_cohort_edit_cohortids::validate_cohortid($cohort1->id, $certification1->id));
        $this->assertNull(form_source_cohort_edit_cohortids::validate_cohortid($cohort2->id, $certification1->id));
        $this->assertNull(form_source_cohort_edit_cohortids::validate_cohortid($cohort3->id, $certification1->id));
        $this->assertNull(form_source_cohort_edit_cohortids::validate_cohortid($cohort1->id, $certification2->id));
        $this->assertNull(form_source_cohort_edit_cohortids::validate_cohortid($cohort2->id, $certification2->id));
        $this->assertNull(form_source_cohort_edit_cohortids::validate_cohortid($cohort3->id, $certification2->id));

        $this->setUser($user2);

        $this->assertSame('Error', form_source_cohort_edit_cohortids::validate_cohortid($cohort1->id, $certification1->id));
        $this->assertNull(form_source_cohort_edit_cohortids::validate_cohortid($cohort2->id, $certification1->id));
        $this->assertNull(form_source_cohort_edit_cohortids::validate_cohortid($cohort3->id, $certification1->id));
        $this->assertSame('Error', form_source_cohort_edit_cohortids::validate_cohortid($cohort1->id, $certification2->id));
        $this->assertNull(form_source_cohort_edit_cohortids::validate_cohortid($cohort2->id, $certification2->id));
        $this->assertNull(form_source_cohort_edit_cohortids::validate_cohortid($cohort3->id, $certification2->id));
