 ...
 */
final class history_upload_options extends \local_openlms\dialog_form {
    protected function definition() {
        ...
        if ($source && has_capability('tool/certify:assign', $context)) {
            $mform->addElement('advcheckbox', 'assign', get_string('history_upload_assign', 'tool_certify'));
            $mform->setDefault('assign', 0);

            $mform->addElement('advcheckbox', 'skipassigned', get_string('history_upload_skipassigned', 'tool_certify'));
            $mform->setDefault('skipassigned', 0);
            $mform->hideIf('skipassigned', 'assign', 'eq', '0');
        }

        $options = ['' => get_string('choosedots')] + $fileoptions;
        $mform->addElement('select', 'timefromcolumn', get_string('history_upload_timefromcolumn', 'tool_certify'), $options);
        ...
        $mform->addElement('select', 'timeuntilcolumn', get_string('history_upload_timeuntilcolumn', 'tool_certify'), $options);
        ...
        $mform->addElement('select', 'timecertifiedcolumn', get_string('history_upload_timecertifiedcolumn', 'tool_certify'), $options);
        ...
        $mform->addElement('select', 'evidencecolumn', get_string('history_upload_evidencecolumn', 'tool_certify'), $options);
        if (in_array('evidence', $fileoptions)) {
            $mform->setDefault('evidencecolumn', array_search('evidence', $fileoptions));
        }

        $mform->addElement('textarea', 'evidencedefault', get_string('evidence_default' , 'tool_certify'));
        $mform->addRule('evidencedefault', get_string('required'), 'required', null, 'client');
        $mform->setDefault('evidencedefault', get_string('evidence_default_text', 'tool_certify'));
        ...
        $this->add_action_buttons(true, get_string('history_upload', 'tool_certify'));
    }
    ...
    public function validation($data, $files) {
        ...
        if ($data['timefromcolumn'] !== '' && $data['timefromcolumn'] === $data['timeuntilcolumn']) {
            $errors['timeuntilcolumn'] = get_string('columnusedalready', 'tool_certify');
        }
