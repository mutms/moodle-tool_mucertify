 ...
 */
final class valid_relateduser extends base {
    /**
     * Send notifications.
     *
    ...
    public static function notify_users(?stdClass $certification, ?stdClass $user): void {
        ...
        $assignment = null;
        $relateduser = null;
        $loadfunction = function(stdClass $period) use (&$certification, &$source, &$assignment, &$user, &$relateduser): void {
            global $DB;
            ...
            if (!$relateduser || $relateduser->id != $period->relateduserid) {
                $relateduser = $DB->get_record('user', ['id' => $period->relateduserid], '*', MUST_EXIST);
            }
        };
        ...
        $sql = "SELECT cp.*, rur.parent as relateduserid
                  ...
                  JOIN {local_openlms_notifications} n
                       ON n.component = 'tool_certify' AND n.notificationtype = 'valid_relateduser' AND n.instanceid = c.id AND n.enabled = 1
                  JOIN {relatedusersrecursive} rur ON rur.fieldid = :fieldid AND rur.child = ca.userid AND rur.depth = 1
             ...
             LEFT JOIN {local_openlms_user_notified} un
                       ON un.notificationid = n.id AND un.userid = rur.parent AND un.otherid1 = ca.id AND un.otherid2 = cp.id
        ...
        foreach ($rs as $period) {
            $loadfunction($period);
            self::notify_related_user($certification, $source, $assignment, $period, $user, $relateduser, false);
        }
        $rs->close();
    }
}
